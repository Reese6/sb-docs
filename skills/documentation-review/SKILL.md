...
4. Не находка: отсутствие опциональных `ui.md`, `api.md`, `technical.md`, `decisions/`; пустая колонка «Покрыто» у feature без этих документов — вход для следующих стадий.
...
2. Проверить каждый документ scope по таблице находок; выполнить все семь групп, порядок внутри неважен.
...
- [ ] Четыре итоговых списка присутствуют (пустые — `none`).
