...
| `## Прочитать` | обязательно → если есть → глобальный контекст → образец | 1000 |
...
- ID ADR берётся из H1: `# ADR-001: <Title>`. В frontmatter `title` двоеточие недопустимо: `title: ADR-001 <Title>`.
