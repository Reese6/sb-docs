...
2. Если есть: `model/`, `ui.md` и `api/` целиком — решение реализует их состав данных, контракт и поведение, не переопределяет; текущий `technical.md`; ADR — глобальные в [docs/architecture/adr/](../../../docs/architecture/adr/README.md) и `docs/features/<feature>/decisions/`.
...
Общие — AGENTS.md, `rules/ai-guardrails.md`. Ядро: текст документа, `title` и H1 — русский (заголовки секций — дословно по шаблону; ID, статусы, `TBD`/`ASSUMPTION` — английские); факт — без пометки, только из `requirements.md`, `ui.md`/`api/`, кода в `services/`, `docs/architecture/` или слов человека; неизвестное — `TBD: <что неизвестно>`; предположение — `ASSUMPTION: <текст>. Requires confirmation.`; у `technical.md` своих ID нет — только ссылки на FR/BR/NFR/UI/API.

1. Два источника правды: `requirements.md` (и `model/`, `ui.md`, `api/`) — требуемое поведение; код в `services/` — текущая реализация. Уровни не смешивать.
...
5. Доменные сущности и поля — в `model/` feature и в `docs/architecture/data-model/`; здесь ссылка на них и физический уровень: таблицы, индексы, миграции. Сущности не переописывать; `model/` нет, а сущности новые — предложить skill `data-model`.
...
10. Внешние endpoints и пользовательские ошибки — `api/`/`ui.md`, acceptance criteria — `requirements.md`; здесь внутренние API, ломающие изменения выделять явно.
...
2. Определить охват: из `requirements.md`, `ui.md`, `api/` выбрать FR/BR/NFR/UI/API, которые реализует решение (правила 4, 6).
...
4. Заполнить каждую секцию шаблона (components; data flow — Mermaid sequenceDiagram или список; services involved с путём в `services/`; data model changes — ссылка на `model/` и физические изменения; API changes; events; caching; transactions; error handling; security; observability; performance; backward compatibility; migration; deployment; testing considerations; alternatives; risks) или `Not applicable: <причина>`.
...
7. Если изменено существующее решение: `grep -rn` по затронутым ID и именам компонентов в `docs/` → расхождения с `requirements.md`, `model/`, `ui.md`, `api/`, ADR → конфликт пометить `TBD`/`ASSUMPTION` → обновить Traceability. Связанные документы молча не переписывать.
...
- [ ] Подготовка выполнена: прочитаны требования, `ui.md`/`api/`, ADR; проверен `services/`.
...
- [ ] Доменные сущности не переописаны — дана ссылка на `model/`; здесь только физический уровень.
