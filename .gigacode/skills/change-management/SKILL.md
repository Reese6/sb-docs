...
Общие — AGENTS.md, `rules/ai-guardrails.md`. Ядро: текст, `title` и H1 — русский, ключевые слова — английские; факт — без пометки, только из постановки, документов или слов человека; неизвестное — `TBD: <что неизвестно>`; предположение — `ASSUMPTION: <текст>. Requires confirmation.` Действует и внутри дельт.
