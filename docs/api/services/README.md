...
- Документ создаётся по шаблону `templates/api.md`; ключ `feature:` из frontmatter шаблона удалить — сервисный документ не принадлежит feature. Один метод — один `##`-блок «METHOD path»; `API-XXX` определяется первой строкой блока в форме `- API-001 (→ FR-012): …`, в заголовок ID не выносится.
...
1. API уровня feature живёт в `docs/features/<feature-name>/api.md`; сюда попадает то, что шире одной feature, — контракт сервиса.
