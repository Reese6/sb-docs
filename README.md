...
Пошаговый workflow сотрудника — от постановки задачи до коммита, с примерами промптов — [WORKFLOW.md](WORKFLOW.md).
...
|---------|-----|---------------|
| Механический | `scripts/validate-docs.mjs` | frontmatter, формат и разрешимость ID, ссылки, структура Markdown |
| Смысловой | `skills/documentation-review` | качество требований, противоречия между документами, терминология, подозрения на галлюцинации |
| Сверка с кодом | `skills/spec-verification` | реализовано ли каждое требование и покрыто ли тестом; что код делает сверх спеки и где ей противоречит |

Первые два уровня работают только с документацией: их трассируемость — doc↔doc. Соответствие документации реальному коду проверяет только третий.

### Сверка документации с кодом

```bash
node scripts/spec-inventory.mjs --scope feature:<feature-name> --format md
```

Скрипт строит машиночитаемый инвентарь требований (формулировки, acceptance criteria, связи, TBD/ASSUMPTION) и снимок подключённого кода — это каркас матрицы трассируемости. Саму сверку выполняет `skills/spec-verification`: прямой проход `spec → code` ищет пропуски, обратный `code → spec` — дрейф и противоречия.

Отчёт — разовый артефакт в `reports/verification/`, в Git не попадает. Требования остаются source of truth: расхождения фиксируются, но никогда не «исправляются» в пользу кода. Без подключённого кода в `services/` прогон возможен, но все вердикты будут `unverifiable`.
...
`services/` предназначена для локального подключения (clone/symlink) репозиториев сервисов, чей исходный код AI может анализировать при написании технической документации и при сверке документации с кодом (`skills/spec-verification`).
...
- Доступ только на чтение: ни один skill ничего в `services/` не изменяет.
